f_plot_mess_new <- function(mess_data,panel,anzahl = 30){
  par(mar = c(5, 5, 4, 6))
  ...
  if (panel == "now"){
    ...
    while(more_plots){
      plot(mess_data$MESS_DATUM[mess_data$STATIONS_ID == station_ids[count]][plot_start:plot_end],
           mess_data$TMK.Lufttemperatur[mess_data$STATIONS_ID == station_ids[count]][plot_start:plot_end],
           pch = 16,type = "b",xlim <- c(min(mess_data$MESS_DATUM[plot_start:plot_end]),max(mess_data$MESS_DATUM[plot_start:plot_end])),
           ylim <- c(min(mess_data$TMK.Lufttemperatur[plot_start:plot_end]),max(mess_data$TMK.Lufttemperatur[plot_start:plot_end])),
